...
class Ubrir {
   ...
   $data = '<?xml version = "1.0" encoding = "UTF-8"?>
	...
	protected function xml_extract_result($xml_string) {
      ...
      $response_status = $parse_it->Response->Status[0];
			switch ($response_status) {
				...
				case '30';
					exit('Неверные данные или заполнены не все поля');
					break;
	...
	protected function xml_extract_status_result($xml, $o_status) 
	{
		...
		if(!empty($parse_it->Response->Order->OrderStatus[0])) {
			...
			elseif((string)$status == (string)$o_status) return true;
			else return false;
			}
		else return false;
	  }
  ...
  protected function send_xml($xml) {
		...
		curl_setopt($ch, CURLOPT_CAINFO, dirname(__FILE__).DIRECTORY_SEPARATOR."certs".DIRECTORY_SEPARATOR."bank.crt");
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 1);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
		curl_setopt($ch, CURLOPT_SSLCERT, dirname(__FILE__).DIRECTORY_SEPARATOR."certs".DIRECTORY_SEPARATOR."user.pem");
		curl_setopt($ch, CURLOPT_SSLKEY, dirname(__FILE__).DIRECTORY_SEPARATOR."certs".DIRECTORY_SEPARATOR."user.key");
